...
| **CRITERIOS INVEST** | **I (Independente):** A tela de seleção pode ser desenvolvida de forma isolada, sem dependência de outras US estarem prontas. Ela funciona como ponto de entrada do operador no sistema. <br><br> **N (Negociável):** O formato da seleção pode ser ajustado durante o desenvolvimento (dropdown, botão grande, lista). O que permanece fixo é a obrigatoriedade de selecionar equipe e esteira antes de qualquer registro. <br><br> **V (Valiosa):** Sem essa US, o operador não consegue iniciar o uso do sistema. Os registros podem se misturar entre as duas equipes e comprometer o resultado da prova. <br><br> **E (Estimável):** É uma tela simples. Tem dois campos de seleção, persistência da escolha durante a sessão e bloqueio quando o operador tenta avançar sem selecionar nada. A estimativa é de 1 a 2 dias de desenvolvimento. <br><br> **S (Pequena):** Tela única, com lógica enxuta. Cabe confortavelmente dentro de uma sprint. <br><br> **T (Testável):** Os três critérios de aceite mostram o que validar. A tela precisa exibir as opções no carregamento, a seleção deve continuar válida nos checkpoints seguintes, e o sistema precisa bloquear o salvamento quando nada estiver selecionado. |
...
| **CRITERIOS INVEST** | **I (Independente):** A funcionalidade pode ser construída em paralelo com a US01. A integração entre as duas é direta e ocorre apenas na junção do fluxo. <br><br> **N (Negociável):** A forma de confirmação do início (botão simples, confirmação dupla, swipe) pode ser ajustada conforme o feedback do Ricardo durante os testes. O que permanece fixo é o timestamp automático, sem entrada manual. <br><br> **V (Valiosa):** Resolve um dos problemas centrais relatados na TAPI. Hoje o operador anota a hora de início à mão e isso gera erros de minuto, especialmente na madrugada. Com timestamp automático, esse erro deixa de existir. <br><br> **E (Estimável):** É um botão de início que chama a API e grava o timestamp do servidor com vínculo à esteira já selecionada. Esforço reduzido, com estimativa de aproximadamente 1 dia. <br><br> **S (Pequena):** Funcionalidade fechada em si mesma. Não tem ramificações que extrapolem o escopo da sprint. <br><br> **T (Testável):** Os critérios de aceite descrevem cenários objetivos. O timestamp deve ser gravado sem input manual e o sistema não pode exigir o nome do atleta no momento da largada. |
...
| **CRITERIOS INVEST** | **I (Independente):** Pode ser desenvolvida em paralelo com a US02, já que a lógica de fim é bem parecida com a de início. A integração entre as duas ocorre apenas no fechamento do fluxo completo de uma corrida. <br><br> **N (Negociável):** O alerta de confirmação antes de encerrar pode aparecer de várias formas (modal, toast, segunda tela), e o formato do campo de km pode ser numérico simples ou com máscara. Esses detalhes podem ser definidos durante o desenvolvimento. <br><br> **V (Valiosa):** Fecha o ciclo de registro de uma corrida. Sem essa US, o sistema não sabe quando o atleta saiu e perde o km final, que é o dado mais importante para somar o total da equipe. <br><br> **E (Estimável):** Tela parecida com a de início, com um campo obrigatório de km e um alerta de confirmação. Esforço previsível, com estimativa de aproximadamente 1 dia. <br><br> **S (Pequena):** Funcionalidade compacta, espelho da US02 com pequenas adições. Cabe confortavelmente em uma sprint. <br><br> **T (Testável):** Os critérios deixam claro o que validar. O timestamp final precisa ser gravado, o km da esteira é obrigatório, e o alerta deve aparecer antes do salvamento para evitar fechamento por toque acidental. |
...
| **CRITERIOS INVEST** | **I (Independente):** Pode ser construída de forma isolada, com dados mockados de turno em andamento. A integração com a US02 ocorre apenas quando o fluxo completo entra em testes. <br><br> **N (Negociável):** O formato do alerta de tempo pode ser visual, sonoro ou os dois. A validação de km regressivo pode bloquear o salvamento ou apenas avisar. Esses detalhes podem ser definidos com o Ricardo durante o desenvolvimento. O que permanece fixo é o intervalo de 5 minutos, definido pela Red Bull. <br><br> **V (Valiosa):** É a função de backup do sistema. Se a esteira travar no meio da corrida, são os checkpoints que preservam os dados acumulados até aquele momento. Sem essa US, o sistema fica vulnerável a uma falha que pode comprometer o trabalho de horas. <br><br> **E (Estimável):** Tela com input numérico, alerta automático por contagem de tempo e validação de km regressivo. A parte do timer exige um pouco mais de cuidado, mas a estimativa é segura. <br><br> **S (Pequena):** Apesar de ter três critérios, todos giram em torno do mesmo input. Cabe dentro de uma sprint sem dificuldade. <br><br> **T (Testável):** Os três critérios descrevem cenários bem específicos. O registro recebe timestamp automático, o alerta visual aparece após 5 minutos sem novo checkpoint, e o sistema bloqueia o salvamento quando o km inserido é menor que o anterior. |
...
| **CRITERIOS INVEST** | **I (Independente):** A tela da Camila pode ser desenvolvida em paralelo com as US do Ricardo, utilizando dados mockados. A integração real ocorre quando os endpoints de checkpoint e fim de turno estão respondendo. <br><br> **N (Negociável):** O layout do painel pode variar bastante (cards grandes, gráficos, tabela) sem afetar o núcleo da US. A frequência de atualização também pode ser ajustada conforme a capacidade da infraestrutura. <br><br> **V (Valiosa):** É a tela que a Camila utiliza para coordenar a prova. Sem ela, a coordenadora perde visibilidade do andamento da competição e não consegue tomar decisões em tempo real, que é o problema que motivou o projeto. <br><br> **E (Estimável):** O sistema soma os checkpoints, atualiza em tempo real (via WebSocket ou polling curto) e exibe os totais na tela. A parte do tempo real é a que exige mais atenção, mas a estimativa é segura. <br><br> **S (Pequena):** Uma tela com dois indicadores principais (total por equipe e total geral) e atualização contínua. Cabe em uma sprint, principalmente se a infraestrutura de tempo real já estiver definida. <br><br> **T (Testável):** Os critérios são objetivos. Os totais aparecem assim que a tela carrega, e qualquer registro novo feito pelo Ricardo é exibido automaticamente sem o usuário precisar atualizar a página. |
