...
Essa tela é acessada a partir da tela "Placar Final", pelo botão de relatório editável. Permite ao operador filtrar os dados por equipe, atleta, esteira ou período de tempo e selecionar quais categorias incluir no relatório. Por padrão, "Resumo geral" e "Detalhamento por turno" vêm marcados. O formato PDF gera um documento com o layout oficial do evento, enquanto CSV e XLSX exportam dados brutos para planilhas. Ao clicar em "EXPORTAR RELATÓRIO", o arquivo é gerado e o download inicia automaticamente. O botão "voltar" retorna à tela de Placar Final.
