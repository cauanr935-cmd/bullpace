...
A Figura 3.3.Y ilustra o wireframe da tela de placar final. Atendendo à necessidade de comparação simultânea entre as equipes (US12), o layout é estruturado em dois painéis organizados lado a lado, cada um exibindo os indicadores de desempenho da respectiva equipe: total de quilômetros percorridos, total de trocas realizadas e velocidade média. O painel da equipe vencedora é renderizado em tamanho maior, reforçando visualmente seu desempenho superior na competição. Além disso, o símbolo de comparação no centro indica qual equipe apresentou maior quilometragem acumulada para facilitação visual.
<br>
<div align="center">
...
A disposição lado a lado dos resultados visa permitir à gestora uma leitura imediata e inequívoca do desfecho da competição, sem necessidade de navegação adicional. Na parte inferior, um atalho direciona ao relatório final do evento, onde métricas e dados completos estão disponíveis para consulta e possíveis alterações. A tela representa o encerramento do fluxo principal da aplicação, consolidando os resultados de ambas as equipes em uma visualização conclusiva.
